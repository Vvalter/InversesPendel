...
#define NUM_MESSUNGEN 7500
static int MESSUNGEN[NUM_MESSUNGEN];
static int MESSUNGEN_POSITION[NUM_MESSUNGEN];
static int messungInd = 0;
        ...
        for (;;) {
                ...
                if (positionWaggon != lastPositionWaggon) {
                        if (currentState == DREHZAHL_MESSUNG) {
                                MESSUNGEN[messungInd] = time;
                                MESSUNGEN_POSITION[messungInd++] = lastPositionWaggon - positionWaggon;
                                if (messungInd > NUM_MESSUNGEN) {
                                        int i = 0;
                                        for (; i < NUM_MESSUNGEN; i++) {
                                                ezs_printf("%d %d\n", MESSUNGEN[i], MESSUNGEN_POSITION[i]);
                                        }
                                        break;
                                }
                                //ezs_printf("%d %d\n", time, lastPositionWaggon - positionWaggon);
                        }
