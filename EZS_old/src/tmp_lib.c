#include <libopencm2/stm32/f4/rcc.h>
#include "lib.h"
...
int initRotaryEncoderTimer(uint32_t tim,  uint32_t portA, uint16_t pinA, uint8_t afA, uint32_t portB, uint16_t pinB, uint8_t afB) {
	...
	enum rcc_periph rccTim = RCC_TIM2;
	switch (tim) {
	    case TIM1: rccTim = RCC_TIM1; break;
	    ...
	    case TIM9: rccTim = RCC_TIM9; break;
	    default: return 1;
	}

	enum rcc_periph rccA;
	switch (portA) {
